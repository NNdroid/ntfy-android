...
import app.fjj.ntfy.R
import app.fjj.ntfy.app.Application
import app.fjj.ntfy.db.Repository
import app.fjj.ntfy.firebase.FirebaseMessenger
import app.fjj.ntfy.msg.NotificationService
import app.fjj.ntfy.util.Log
import app.fjj.ntfy.util.topicUrl
import java.io.InputStreamReader

class Backuper(val context: Context) {
    private val gson = Gson()
    private val resolver = context.applicationContext.contentResolver
    private val repository = (context.applicationContext as app.fjj.ntfy.app.Application).repository
    private val messenger = app.fjj.ntfy.firebase.FirebaseMessenger()
    private val notifier = NotificationService(context)

    suspend fun backup(uri: Uri, withSettings: Boolean = true, withSubscriptions: Boolean = true, withUsers: Boolean = true) {
        Log.d(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Backing up settings to file $uri")
        ...
        Log.d(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Backup done")
    }

    suspend fun restore(uri: Uri) {
        Log.d(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Restoring settings from file $uri")
        val reader = JsonReader(InputStreamReader(resolver.openInputStream(uri)))
        val backupFile = gson.fromJson<app.fjj.ntfy.backup.BackupFile>(reader, app.fjj.ntfy.backup.BackupFile::class.java)
        applyBackupFile(backupFile)
        Log.d(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Restoring done")
    }
    ...
    private suspend fun applyBackupFile(backupFile: app.fjj.ntfy.backup.BackupFile) {
        if (backupFile.magic != app.fjj.ntfy.backup.Backuper.Companion.FILE_MAGIC) {
            throw app.fjj.ntfy.backup.InvalidBackupFileException()
        }
    ...
    private fun applySettings(settings: app.fjj.ntfy.backup.Settings?) {
        if (settings == null) {
            return
        }
    ...
    private suspend fun applySubscriptions(subscriptions: List<app.fjj.ntfy.backup.Subscription>?) {
        ...
        subscriptions.forEach { s ->
            try {
                // Add to database
                val subscription = app.fjj.ntfy.db.Subscription(
                    id = s.id,
                    baseUrl = s.baseUrl,
                    topic = s.topic,
                    instant = s.instant,
                ...
                Log.w(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Unable to restore subscription ${s.id} (${topicUrl(s.baseUrl, s.topic)}): ${e.message}. Ignoring.", e)
            }
        }
    }

    private suspend fun applyNotifications(notifications: List<app.fjj.ntfy.backup.Notification>?) {
        ...
        notifications.forEach { n ->
            try {
                ...
                repository.addNotification(app.fjj.ntfy.db.Notification(
                    id = n.id,
                ...
                Log.w(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Unable to restore notification ${n.id}: ${e.message}. Ignoring.", e)
            }
        }
    }

    private suspend fun applyUsers(users: List<app.fjj.ntfy.backup.User>?) {
        if (users == null) {
            return
        }
        users.forEach { u ->
            try {
                repository.addUser(app.fjj.ntfy.db.User(
                    baseUrl = u.baseUrl,
                    username = u.username,
                    password = u.password
                ))
            } catch (e: Exception) {
                Log.w(app.fjj.ntfy.backup.Backuper.Companion.TAG, "Unable to restore user ${u.baseUrl} / ${u.username}: ${e.message}. Ignoring.", e)
            }
        }
    }

    private suspend fun createBackupFile(withSettings: Boolean, withSubscriptions: Boolean, withUsers: Boolean): app.fjj.ntfy.backup.BackupFile {
        return app.fjj.ntfy.backup.BackupFile(
            magic = app.fjj.ntfy.backup.Backuper.Companion.FILE_MAGIC,
            version = app.fjj.ntfy.backup.Backuper.Companion.FILE_VERSION,
    ...
    private fun createSettings(): app.fjj.ntfy.backup.Settings {
        return app.fjj.ntfy.backup.Settings(
    ...
    private suspend fun createSubscriptionList(): List<app.fjj.ntfy.backup.Subscription> {
        return repository.getSubscriptions().map { s ->
            app.fjj.ntfy.backup.Subscription(
                id = s.id,
                baseUrl = s.baseUrl,
                topic = s.topic,
                instant = s.instant,
    ...
    private suspend fun createNotificationList(): List<app.fjj.ntfy.backup.Notification> {
        return repository.getNotifications().map { n ->
            val actions = if (n.actions != null) {
                n.actions.map { a ->
                    app.fjj.ntfy.backup.Action(
                        id = a.id,
                        action = a.action,
                        label = a.label,
                        clear = a.clear,
            ...
            val attachment = if (n.attachment != null) {
                app.fjj.ntfy.backup.Attachment(
                    name = n.attachment.name,
                    type = n.attachment.type,
                    size = n.attachment.size,
                    expires = n.attachment.expires,
            ...
            app.fjj.ntfy.backup.Notification(
                id = n.id,
                subscriptionId = n.subscriptionId,
                timestamp = n.timestamp,
                title = n.title,
    ...
    private suspend fun createUserList(): List<app.fjj.ntfy.backup.User> {
        return repository.getUsers().map { u ->
            app.fjj.ntfy.backup.User(
                baseUrl = u.baseUrl,
                username = u.username,
                password = u.password
            )
...
data class BackupFile(
    val magic: String,
    val version: Int,
    val settings: app.fjj.ntfy.backup.Settings?,
    val subscriptions: List<app.fjj.ntfy.backup.Subscription>?,
    val notifications: List<app.fjj.ntfy.backup.Notification>?,
    val users: List<app.fjj.ntfy.backup.User>?
)
...
data class Notification(
    ...
    val icon: app.fjj.ntfy.backup.Icon?,
    val actions: List<app.fjj.ntfy.backup.Action>?,
    val attachment: app.fjj.ntfy.backup.Attachment?,
    val deleted: Boolean
)
